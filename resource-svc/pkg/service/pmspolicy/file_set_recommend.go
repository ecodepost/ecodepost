...
type FileSetRecommend struct {
}

func init() {
	Register(NewActPolicy(&FileSetRecommend{}, superAdminCheckFn))
}

func (*FileSetRecommend) Scheme() commonv1.PMS_ACT {
	return commonv1.PMS_FILE_SET_RECOMMEND
}

// Check 检查
// 1 超级管理员
// 2 用户组中是否设置里权限
func (s *FileSetRecommend) Check(ctx context.Context, uid int64, fileGuid string) (bool, error) {
	// 文件信息
	...
	if err != nil {
		return false, fmt.Errorf("FileSetRecommend get roleIds fail, err: %w", err)
	}
	if len(roleIds) == 0 {
		return false, nil
	}
